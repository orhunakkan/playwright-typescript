---
name: run-tests
description: Commands for running the current Playwright docs-monitoring suite, listing tests, focusing failures, updating text snapshots, and opening reports.
---
...
As of 2026-05-11, local discovery reports Playwright `1.59.1` and 396 collected tests across the two configured projects.
...
```bash
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --grep "sidebar links match baseline"
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --grep "playwright-dev-docs-test-cli"
```
...
```bash
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --headed
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --ui
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --debug
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --trace on
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --workers=1
npx playwright test tests/scrapper/playwright-docs-link-monitoring.spec.ts --project="Desktop Chrome" --repeat-each=2
```
...
## JSON Baseline Validation

After editing the sidebar baseline:
...
Do not suggest old npm scripts or commands for removed framework layers. The active suite is the docs-monitoring spec under `tests/scrapper/`.

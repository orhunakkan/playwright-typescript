...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `C:\Users\orhun\.claude\plans\` if a plan was mentioned).
   ...
   - Memory files written or updated (`C:\Users\orhun\.claude\projects\c--Users-orhun-playwright-typescript\memory\`).
...
- Docker containers: <container name/service + how to confirm running: `docker ps`> — or "none" (note: DB tests require `npm run docker:up` before `npm run test:db`)
- Open worktrees / branches: <paths> — or "none"

## Environment prerequisites
- `.env` present and populated: PRACTICE_E2E_URL, PRACTICE_API_URL — required before running any tests
- <any other env-specific state the next agent needs to know>

## Verification — how to confirm things still work
- `npm run typecheck` — zero TypeScript errors
- `npm run lint:check` — zero lint errors
- `npx playwright test --project="Desktop Chrome" <spec-file>` — tests worked on this session pass
- `npm run test:db` — DB tests pass (requires Docker running: `npm run docker:up` first)
- <add session-specific commands if other files were touched>
...
7. **Docker container state is running state.** If the PostgreSQL container was started this session (`npm run docker:up`), record it in "Running state". If it is still up, the next agent inherits it; if it was torn down, say so.
...
- Skipping the "Environment prerequisites" section — even if .env is obviously present, confirm it.
...
- Using generic verification commands. Always include the specific spec file or project flag that matches what was worked on this session.

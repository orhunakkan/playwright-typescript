...
test.describe('Dropdown Menu Testing', () => {
  test('Dropdown Menu Item Hover States', async ({ page }) => {
    ...
    // 3. Hover over "Another action" menu item
    await page.getByRole('link', { name: 'Another action' }).hover();

    // 4. Verify hover state is active (element is still visible after hover)

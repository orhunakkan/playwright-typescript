...
- **Notable finding during test design:** `role="status"` only derives its accessible _name_ from an explicit `aria-label` per the ARIA spec, not from text content — the product-count `<span role="status">` has no `aria-label`, so `getByRole('status', { name: ... })` matches zero elements even though the correct text is present; the POM instead scopes `statusRegion` to `span[role="status"]` to disambiguate it from the transient loading `<div role="status" aria-label="Loading products">` that briefly coexists during a Reload-triggered refetch
